...
import { validationUpdateMovieSchema } from "../schemas/movie";
import { useState } from "react";
import { toast } from "sonner";
import { useMovieOperations } from "../hooks/useMovieOperation";
import type { MovieUpdateRequest, MovieResponse } from "../types/movie";
...
export function EditMovie({ movie, onSuccess }: EditMovieProps) {
  const [open, setOpen] = useState(false);
  const { updateMovie, isLoading } = useMovieOperations();

  async function handleSubmit(
    values: MovieUpdateRequest & { poster: File | string }
  ) {
    try {
      const movieData: MovieUpdateRequest = {
        id: movie.id,
        originalTitle: values.originalTitle,
        portugueseTitle: values.portugueseTitle,
      ...
      const success = await updateMovie(movieData, values.poster as File);
  ...
  const formik = useFormik({
    ...
    validationSchema: validationUpdateMovieSchema,
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          variant="secondary"
          className="bg-mauve-dark-300/60 text-mauve-dark-1200 w-full lg:w-fit"
        >
          Editar
        </Button>
      </DialogTrigger>
        ...
        <div className="flex justify-between items-start space-y-4">
          <div>
            ...
            <DialogDescription className="text-mauve-dark-1100 mt-2">
              Edite as informações do filme conforme necessário.
            </DialogDescription>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="mt-2 space-y-2">
                ...
                <label
                  htmlFor="poster"
                  className="bg-mauve-dark-200 border rounded border-mauve-dark-600 p-4 w-full h-11 font-roboto font-normal text-[16px] text-mauve-dark-1200 focus-within:border-purple-dark-900 cursor-pointer flex items-center gap-2 hover:border-purple-dark-900 transition-colors"
                >
